...
def test_feed_multiple_times():
    ...
    pet.feed(1)
    pet.feed(1)
    assert pet.status['hunger'] == 1

def test_exercise():
    """Tests exercising under normal conditions"""
    pet = Pet("Hallie", "cat")
    pet.status['boredom'] = 5
    pet.status['happiness'] = 5
    pet.status['hunger'] = 5
    pet.exercise(1)
    assert pet.status['boredom'] == 4
    assert pet.status['happiness'] == 6
    assert pet.status['hunger'] == 6
...
def test_exercise_max_hunger():
    ...
    pet.status['boredom'] = 5
    pet.status['happiness'] = 5
    pet.status['hunger'] = 10
    pet.exercise(1)
    assert pet.status['boredom'] == 5
    assert pet.status['happiness'] == 5
    assert pet.status['hunger'] == 10
